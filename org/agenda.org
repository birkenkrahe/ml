...
Thu: ???
#+begin_notes
1) Creating and interpreting correlation and scatterplot matrix - all
   features correlated against one another.
2) Special scatterplot matrix with ~psych::pairs.panels~ showed
...
3) Modeling with ~stats::lm~ and formula ~y ~ .~, ~y ~ x1 + x2~, ~y ~ x1 * x2~
   #+begin_example R
   ...
   #+end_example
4) Prediction with ~stats::predict~
   #+begin_example R
     p <- predict(model, test) # test data as (unseen) input for model
   #+end_example
5) Factor features are automatically dummy-coded with the first level
...
- [ ] [[https://lyon.instructure.com/courses/1021/pages/sign-up-for-final-presentations][Three teams have not yet signed up in Canvas]]: no matter, present
  what you have achieved - we're not looking for perfection but for
  diligence - do you know the difference?[fn:2]
- [ ] Last two DataCamp assignments are *bonus* assignments (complete by
  May 3rd)
- [ ] Test preview/review questions: decision trees
...
complete plenty of fun projects and document them. 4) Remember: what
you'll apply will grow, what you won't use will wither on the vine.
