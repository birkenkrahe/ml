...
* TODO Week 6 - ML models overview
#+attr_latex: :width 400px
[[../img/ml_types2.png]]

- [ ] Remember to uppload practice (there are deadlines) [[https://lyon.instructure.com/courses/1021/assignments][to Canvas]]
- [ ] Open ~2_R_explore_practice.org~ and load the data
- [ ] Let's finish the review and upload the completed file to Canvas
- [ ] What is R? Good overview [[https://www.datacamp.com/blog/all-about-r][in this DataCamp blog post]] (05/22)
- [ ] [[https://lyon.instructure.com/courses/1021/assignments/6997][Test 2 (open book) is live online until Fri 17-Feb, 11:59 pm]].
* Review: exploring numerical data structure:

Open an Org-mode file if you want to code along.

1) [ ] How can you get an overview of statistical information for ~Nile~?
   #+begin_src R :results output
     summary(Nile)
   #+end_src
2) [ ] What about the ~time~ of the ~Nile~ observations?
   #+begin_src R :results output
     summary(time(Nile))
   #+end_src
3) [ ] How many values of ~Nile~ are outliers? Which values are these?
   #+begin_src R :results output
     length(which(Nile > (IQR(Nile) * 1.5)))
...
6) [ ] How many observations of the ~Nile~ flow are between 800 and 1000
   mio cubic metres?
   #+begin_src R
     length(Nile[which(Nile > 800 & Nile < 1000)])
   #+end_src
* References
