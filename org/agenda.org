...
* NEXT Week 12: Spam filter & intro to regression
#+attr_latex: :width 400px
...
- Load the ~.RData~ file from GitHub into your current R session and
  check that packages and user-defined objects were loaded (make sure
  the location is correct - this ~load~ command assumes ~.RData~ is in the
