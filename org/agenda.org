...
* Review: kNN DataCamp lesson

1) [ ] Load the ~signs~ dataset from the lesson - how can you get it?
   #+begin_src R :results output
     signs <- read.csv(file="../data/knn_traffic_signs.csv",
                       stringsAsFactors=TRUE)
     str(signs[1:4])
   #+end_src

2) [ ] What information does the dataset ~signs~ contain?
   #+attr_latex: :width 400px
   [[../img/stop.png]]
   #+begin_notes
   - Street signs were divided into 4x4=16 panel grids
   - Each grid's RGB levels, location and type were recorded
   #+end_notes

3) [ ] For the classification task, what is the class?

4) [ ] How many types of ~signs~, how many of each sign type are there?
   #+begin_src R :results output
     table(signs$sign_type)
   #+end_src

5) [ ] Which function and which package was used for kNN?
   #+begin_src R :results output
     library(class)  # load package
     search()  # check package search path
     ls('package:class') # check package functions
   #+end_src
   ...
   :  [1] "batchSOM"  "condense"  "knn"       "knn.cv"    "knn1"      "lvq1"     
   :  [7] "lvq2"      "lvq3"      "lvqinit"   "lvqtest"   "multiedit" "olvq1"    
   : [13] "reduce.nn" "SOM"       "somgrid"

6) [ ] What were the three arguments for the classifier?
   #+begin_src R
     knn(train = signs[-c(1:3)],     # training data
         test = signs[206, -c(1:3)], # test data
         cl = sign_types)            # class
   #+end_src
...
7) [ ] Which conditions should the training/test data fulfil?
   #+begin_notes
   - They should be randomized and not grouped or ordered
   - The id column needs to be removed
   - Each subset needs to be representative of the full dataset
   #+end_notes

8) [ ] Which condition should the features fulfil?
   #+begin_notes
     - They need to be normalized or standardized by scale.
     - They need to be independent of one another.
     - They need to be characteristic of the class levels
   #+end_notes

9) [ ] What are characteristics of the target class?
   #+begin_notes
     The class is a ~factor~ vector whose ~levels~ are the class values.
   #+end_notes

10) [ ] In the ~r10~ feature (10th grid location), what are the average
    values in each target class? 
    #+begin_src R :results output
      tapply(X = signs$r10,
             INDEX = signs$sign_type,
             FUN = mean)
    #+end_src
