...
2) [ ] What is used for "training" the k-NN classifier?
3) [ ] What is used to test the "trained" k-NN classifier?
4) [ ] What is the meaning of the parameter 'k'?
   #+begin_notes
    - The number of nearest neigbors when computing distance
    - A measure of the size of the classification neighborhood
    - An argument in the ~class::knn~ function
   #+end_notes
5) [ ] What are the arguments of ~knn~?
   #+begin_src R
     args(knn)
   #+end_src
6) [ ] What is the purpose of splitting the data in training and test
   data?
7) [ ] What happens when voting is tied after measuring the distances?
8) [ ] If I have 10 features q_{1}...q_{10} and 1 target label
   ...
   - Each point has 10 coordinates. The distance formula has 10 terms
     p_{i}-q_{ij}, one for each i-th coordinate of q_{j}, and 11 variables are
     involved: 10 features (predictor) and 1 target variable
     (predicted).
   #+end_notes
9) [ ] Is a bigger ~k~ always better? Why or why not?
   #+begin_notes
   ...
   #+end_notes
10) [ ] Measuring model performance:
    - What is the confusion matrix? What's its dimension?
    - How can you compute the confusion matrix for kNN?
    - How can you assess the accuracy numerically?
    #+begin_notes
    - The confusion matrix is a 2 x 2 cross-table if the target
      feature had 2 labels/categories (e.g. male/female or
      benign/malignant), or a 3 x 3 for 3 labels. It shows, which
      levels were accurately identified and which were not.
